...
function OfertaEducativa() {
  const carreras: Carrera[] = [
    {
      ...
      logo: LogoIA,
      division:
        "DIVISIÓN DE INFORMACIÓN, INTELIGENCIA ARTIFICIAL Y COMUNICACIÓN",
    },
    {
      id: 2,
      label: "ITIID",
  ...
  const divisiones = Array.from(
    new Set(carreras.map((carrera) => carrera.division)),
  );
  ...
  const getDivisionText = (division: string): string => {
    ...
    if (isDesktop) {
      ...
      if (
        sinPrefijo === "INFORMACIÓN, INTELIGENCIA ARTIFICIAL Y COMUNICACIÓN"
      ) {
  ...
  return (
    <>
      ...
      <h1>
        <span className="Text1" style={{ color: "#01433f" }}>
          DIVISIONES DE
        </span>
        <span className="Text2" style={{ color: "#18817d" }}>
          CARRERAS
        </span>
      </h1>
        ...
        <div className="filtrosContainer">
          ...
          >
            ...
            {divisiones.map((division, index) => {
              const cantidad = carreras.filter(
          ...
          <div className="scrollIndicators">
            {!isDesktop && canScrollRight && (
              <span className="scrollHint">Desliza →</span>
            )}
          </div>
        </div>
          ...
          <div className="carrerasGrid">
            {carrerasFiltradas.map((carrera) => (
              <div key={carrera.id} className="carreraCard">
                <a
                  href={`/Carrera/${carrera.label}`}
                  className="carreraLogoContainer"
                >
                  <img
                    src={carrera.logo}
